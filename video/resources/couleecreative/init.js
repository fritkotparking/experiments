...
if($('#page-header-bg[data-parallax="1"]').length > 0) {
	...
	$(window).scroll(function(){
		...
		if(!$('body').hasClass('mobile') && navigator.userAgent.match(/iPad/i) == null){
			...
			$('#page-header-bg[data-parallax="1"] .span_6').stop(true,true).transition({ y: $scrollTop*-.14 },0);
...
/***************** Nav ******************/
	...
	function toggleNavClass(){
		...
		$('#header-outer').toggleClass( 'minimized', scrollTop > 100 ); 
	...
	//responsive nav
	$('#toggle-nav, #mobile-menu .close-mobile-menu' ).click(function(e){
		e.preventDefault();
		if ( $('#toggle-nav').hasClass('open') ){
			$('#toggle-nav').removeClass('open'); 
			$('#mobile-menu').stop(true,true).slideUp(400, function(){
				$('#mobile-menu .close-mobile-menu').hide();

			});			
		} else {
			$('#toggle-nav').addClass('open'); 
			$('#mobile-menu').stop(true,true).slideDown(400, function(){
				$('#mobile-menu .close-mobile-menu').fadeIn('slow');
			});		
		}
	});
...
/*-------------------------------------------------------------------------*/	
	...
	/* ==== tweaks ============================================= */
	// two-section iphorm form
	( function(){
		var $switch = $('#switch-form-type');
		var $switches = $switch.find( 'a' );
		var $target = $('.iphorm .iphorm_1_1-element-wrap select');
		$switches.on( 'click.cc', function(e){
			e.preventDefault(); 
			$switches.removeClass('active');
			$(this).addClass('active');
			var index = $(this).index();
			var newVal = $target.find( 'option:eq('+index+')' ).val();
			$target.val( newVal ).change();	
					
		});
		$('.iphorm').each( function(){
			var $form = $(this);
			var $inputs = $(this).find( 'input, textarea');
			$inputs.on( 'focus.cc', function(){
				var $container = $(this).parents('.iphorm-element-wrap').first();
				$container.removeClass('iphorm-element-error');
				$container.find( '.iphorm-errors-wrap').fadeOut('fast');
			});
			// set up budget select
			( function(){
				var $budgetSelect = $form.find( 'select.iphorm_1_15, select.iphorm_3_15');	
				var html = '<ul class="select-options cf">';
				$budgetSelect.find( 'option' ).each( function(){
					html += '<li class="select-option" data-value="'+ $(this).val() + '"><a class="cc-button">'+ $(this).text() + '</a></li>'
				});
				html += '</ul>';
				var $optionsContainer = $( html ).insertAfter( $budgetSelect );
				$optionsContainer.find( 'li').each( function(){
					var $item = $(this);
					$item.find( 'a' ).on( 'click.cc', function(e){
						e.preventDefault();
						$budgetSelect.val( $item.data('value')).change();
						$optionsContainer.find( 'li.active').removeClass('active');
						$item.addClass('active');
					});
				})
				$budgetSelect.on( 'change.cc', function(){
					$optionsContainer.find( 'li' ).removeClass('active');
					$optionsContainer.find( 'li[data-value="' + $(this).val() + '"]' ).addClass('active');
				});
				$budgetSelect.prop( 'selectedIndex', -1 );
				// select first item
				// setTimeout( function(){
				// 	$optionsContainer.find( 'li' ).first().find( 'a' ).trigger( 'click' );
				// }, 100);
	...
	$('.wpcf7').each( function(){
		$('input,textarea', $(this)).on( 'focus', function(){
	...
	$('.nectar-slider-wrap').each(function(i){
		if ( $(this).find( '.mobile-video-image' ).size() > 0 && $(this).find( '.video-wrap').size() == 0 ){
			$(this).addClass( 'video-fallback' );
		}
		if ( $(this).children( '.swiper-container' ).size() > 0 ){
			$(this).addClass( 'image-slide' );
		}		
	}); 
	setTimeout( function(){
		$('body.single-post .nectar-slider-wrap').insertBefore( '.container-wrap' );
	}, 5000 );

	// blog/archive category switcher
	$('body.blog .categories select, body.archive .categories select').chosen({
		disable_search_threshold: 1000
	});
	$('body.blog .categories select, body.archive .categories select').on( 'change', function(){
